...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QTabWidget, QWidget, QVBoxLayout,
                             ...
                             QDialogButtonBox, QStackedWidget, QFrame, QScrollArea, QMessageBox,
                             QListWidgetItem)
...
class ModernAuthDialog(QDialog):
    ...
    def handle_register(self):
        ...
        if success:
            ...
            QMessageBox.warning(self,"Ошибка регистрации", "Ошибка")
...
class ModernMainWindow(QMainWindow):
    def __init__(self):
        ...
        if auth_dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.setWindowTitle(f"GPT Bot - {self.username} ({self.role})")
        else:
            sys.exit(0)
    ...
    def setup_chat_tab(self):
        ...
        # self.message_input = CustomTextEdit(send_callback=self.send_message)
    ...
    def setup_balance_tab(self):
        ...
        rules_text = QLabel(
            ...
            " <b>Текущие цены:</b><br>"
            " Входные токены: 0.02 руб/1000<br>"
            " Выходные токены: 0.09 руб/1000</b><br>"
    ...
    def on_tab_changed(self, index):
        # if index == self.tabs.indexOf(self.chat_widget):
        #     self.message_input.setFocus()
        if index == self.tabs.indexOf(self.balance_widget):
            self.update_balance_info()
        ...
        for sender, content in history:
            if sender == 'user':
                html += (f"<div style='margin: 10px; "
                         f"padding: 12px; background: #87CEEB ; "
                         f"border-radius: 12px; max-width: 70%; "
                         f"margin-left: auto;'>")
                html += f"<b>Вы:</b><br>{content}</div>"
            else:
                html += (f"<div style='margin: 10px; "
                         f"padding: 12px; background: #f1f5f9; "
                         f"border: 1px solid #e5e7eb; color: #0f172a; "
                         f"border-radius: 12px; max-width: 70%;'>")
    ...
    def start_new_chat(self):
        ...
        if self.role == 'banned':
            QMessageBox.warning(self, "Доступ запрещен ", "Вы забанены !")
            return
    ...
    def process_payment(self):
        ...
        if self.role == 'banned':
            QMessageBox.warning(self,"Вы заблокированны", "Нельзя пополнить баланс !")
            return
    ...
    def send_support_message(self):
        ...
        if self.role == 'banned':
            QMessageBox.warning(self, "Вы заблокированны", "Невозможно боратиться в поддержку")
            return
